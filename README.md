...
| `style-caption-overlay` | `overlay_style_caption` | 情绪图叠字（2026-09-04 入册）：风格图（桶里）+ 页面文案 → **画面原样贴进一张更高的纸**、上下白边由文案实测高度算出来、标题/总结/贴士落在白边里 → 写回同前缀 `…-captioned.png` |
...
## 情绪图叠字的版面（`style_caption`）

**版面自己造，不向模型要**（用户裁决 2026-09-07："把'留白'从'求模型'改成'自己做'。
生成侧不再要求留白带"）。此前是"叠字前量版面，量不到足够的连续空白就整张失败去重生成"，
而留白是 imagegen 模板里一句英文向模型讨的——**模型给多少是它的事**。9-07 真机两跑的代价：
一跑三次全判不过、业主一张图没收到；另一跑第三次才过，7 分 26 秒里 7 分钟耗在重生循环上。

现在的做法与口径：

- **画面一个像素不动**：不缩放、不拉伸、不裁切，原尺寸贴上去。成图宽度＝画面宽度
  （画面左右照旧出血到页边，读起来是"图还往外延伸"而不是"被裁了一刀"）；
  成图高度＝画面高度＋上下两条白边，**比例随文案长短变**——凑固定比例就得缩画面。
- **白边高度是算出来的**：标题/总结/贴士按页宽排完之后的实测高度，上下各加一份
  `TEXT_PAD_SHARE` 余量。文案再长白边跟着长高，**放不下这回事不存在，重试次数恒为 0**。
- **纸接得上**：白边底色取画面**那一侧边缘**的中位色（不是整张一个色——真图实测底边比顶边
  暗，是画的落影），再把画面里最长一段纯纸的纸纹镜像铺到接缝处、往外淡尽。纯色版实测过：
  放大三倍能看到接缝那一线由"有颗粒"变"死平"；铺纸纹后同样放大三倍看不出接缝也看不出重复。
- **`StyleCaptionError` 仍在**：改完它理论上不再触发，还能触发的只剩"图窄到一个字都放不下
  一行"这一类。真触发了说明别处有问题——**整张不出，不裁字不缩字不静默兜底**。
...
5. **叠字不动业主家的画面**：只许原样放置或等比缩放，压扁/拉伸/裁切一律算画错。
